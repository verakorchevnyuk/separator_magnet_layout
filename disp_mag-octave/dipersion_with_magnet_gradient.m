function [alpha1, alpha2, total_alpha] = dipersion_with_magnet_gradient( Bgrad, plot_info )
...
switch plot_info
    case 'yes'
%         xx_cut(:) = linspace(0,P0(end,1)*1.25,100) ;
%         fy = @(m,b,x) m*x + b ;
...
%         plot(xx_cut(:),yy_cut(:),'.'); % Cutting line
        ...
        title(['Beta = ', num2str(beta), ' degrees. Bgrad = ', num2str(Bgrad), ' T/m']);
    otherwise
        return
end
